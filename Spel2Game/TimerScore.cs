...
{/*
	Kortast tid vinner!
	modifierade HighScore class vi fick till spaceshooter. Sortering ändrad till fokusera små tal, och scoren bytt till double för decimaler på sekunderna
	*/
    class HSItem
    {
		...
		public double Seconds { get { return points; } set { points = value; } }

		// =======================================================================
		// HSItem(), klassens konstruktor
		// =======================================================================
		public HSItem(string name, double points)
		{
			this.name = name;
			this.points = points;
		}
	...
	class TimerScore
	{
		...
		// =======================================================================
		public TimerScore(int maxInList)
		{
		...
		void Sort()
		{
			...
			for (int i = 0; i < max; i++)
			{
				...
				{
					if (highscore[j].Seconds > highscore[j + 1].Seconds) // Jämför elementen
					{
						// Byt plats!
		...
		void Add(double seconds)
		{
			...
			HSItem temp = new HSItem(name, seconds);
		...
		// =======================================================================
		public void PrintDraw(SpriteBatch spriteBatch, SpriteFont font, Vector2 position, int lvl)
		{
			string text = "HIGHSCORE LVL " + (lvl+1) +"\n";
			foreach (HSItem h in highscore)
            {
				int minuter = (int)h.Seconds / 60;
				double sekunder = Math.Round(h.Seconds - minuter*60, 3);

				text += h.Name + " " + minuter + " minuter, " + sekunder + " sekunder\n";

            }

			spriteBatch.DrawString(font, text, position, Color.White, 0, new Vector2(0,0),0.5f,SpriteEffects.None,0);
		}
		...
		// =======================================================================
		public bool EnterUpdate(GameTime gameTime, double time)
		{
		...
		// =======================================================================
		public void EnterDraw(SpriteBatch spriteBatch, SpriteFont font, Vector2 position)
		{
			string text = "Enter name to join Leaderboards: \n" + name + currentChar;
		...
		public void SaveToFile(string filename)
		{
			...
			{
				string text = item.Name + ":" + item.Seconds;
				sw.WriteLine(text);
			}
		...
		public void LoadFromFile(string filename)
		{
			...
			while ((row = sr.ReadLine()) != null)
			{
				...
				double gameTime = Convert.ToDouble(words[1]);
				// Lägg till i listan:
				HSItem temp = new HSItem(words[0], gameTime);
				highscore.Add(temp);
			}
